...
private:
	...
	aiVector3D calcInterpolatedPosition(float p_animation_time, const aiNodeAnim* p_node_anim)
	{
		...
		uint index = findPosition(p_animation_time, p_node_anim);
		uint nextIndex = index + 1;

		if (nextIndex >= p_node_anim->mNumPositionKeys)
			return p_node_anim->mPositionKeys[index].mValue;

		float t1 = (float)p_node_anim->mPositionKeys[index].mTime;
		float t2 = (float)p_node_anim->mPositionKeys[nextIndex].mTime;
		float delta = t2 - t1;

		if (delta <= 0.0f)
			return p_node_anim->mPositionKeys[index].mValue;

		float factor = (p_animation_time - t1) / delta;
		if (factor < 0.0f) factor = 0.0f;
		if (factor > 1.0f) factor = 1.0f;

		const aiVector3D& start = p_node_anim->mPositionKeys[index].mValue;
		const aiVector3D& end = p_node_anim->mPositionKeys[nextIndex].mValue;
	...
	aiQuaternion calcInterpolatedRotation(float p_animation_time, const aiNodeAnim* p_node_anim)
	{
		...
		uint index = findRotation(p_animation_time, p_node_anim);
		uint nextIndex = index + 1;

		if (nextIndex >= p_node_anim->mNumRotationKeys)
			return p_node_anim->mRotationKeys[index].mValue;

		float t1 = (float)p_node_anim->mRotationKeys[index].mTime;
		float t2 = (float)p_node_anim->mRotationKeys[nextIndex].mTime;
		float delta = t2 - t1;
		...
		const aiQuaternion& start = p_node_anim->mRotationKeys[index].mValue;
		const aiQuaternion& end = p_node_anim->mRotationKeys[nextIndex].mValue;
	...
	aiVector3D calcInterpolatedScaling(float p_animation_time, const aiNodeAnim* p_node_anim)
	{
		...
		if (next_scaling_index >= p_node_anim->mNumScalingKeys)
		{
			...
			return p_node_anim->mScalingKeys[scaling_index].mValue;
		}

		float time1 = (float)p_node_anim->mScalingKeys[scaling_index].mTime;
		float time2 = (float)p_node_anim->mScalingKeys[next_scaling_index].mTime;

		float delta_time = time2 - time1;
		...
		float factor = (p_animation_time - time1) / delta_time;
